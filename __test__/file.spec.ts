...
test('new fails for nonexistent path', () => {
  ...
  expect(() => new File(path, false)).toThrow()
})
...
test('new fails when parent is file', () => {
  ...
  expect(() => new File(child, true)).toThrow()
})

test('new rejects directory path', () => {
  const dir = tmpDir()
  expect(() => new File(dir, false)).toThrow()
})
...
test('size returns zero for empty file', () => {
  const dir = tmpDir()
  const file = new File(join(dir, 'empty.txt'), true)
